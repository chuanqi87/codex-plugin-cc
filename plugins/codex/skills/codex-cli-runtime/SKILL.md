...
- Preserve `--backend codex` or `--backend opencode` when the user explicitly selects a backend.
- Leave `--backend` unset by default.
...
- If the forwarded request includes `--backend`, pass it through to `task` and remove it from the natural-language prompt.
